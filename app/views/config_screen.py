...
class SwatchCor(Button):
    """
    Widget customizado que representa uma amostra de cor selecionável.
    """
...
class DialogCor(MDBoxLayout):
    """
    Caixa de diálogo que exibe uma grade de SwatchCor para o usuário 
...
class PreviewCor(Button):
    """
    Widget não selecionável que serve apenas para mostrar a cor 
    atualmente selecionada por um jogador na tela de configuração.
    """
...
class ConfigScreen(MDScreen):
    """
    Tela onde o usuário insere os nomes dos jogadores, escolhe suas cores
    e decide se jogará contra a máquina ou outro humano.
    """
    def __init__(self, **kwargs):
        ...
        corpo.add_widget(MDLabel(
            text=Textos.LBL_JOGADOR_1, font_style="H6",
            theme_text_color="Primary",
            size_hint_y=None, height=dp(28),
        ))
        self.campo_j1 = MDTextField(hint_text=Textos.HINT_JOGADOR_1)
        ...
        self.lbl_jogador2 = MDLabel(
            text=Textos.LBL_JOGADOR_2, font_style="H6",
            theme_text_color="Primary",
            size_hint_y=None, height=dp(28),
        )
        corpo.add_widget(self.lbl_jogador2)
        self.campo_j2 = MDTextField(hint_text=Textos.HINT_JOGADOR_2)
    ...
    def _on_ia_active(self, checkbox, value):
        if value:
            self.campo_j2.text = Textos.NOME_IA
        ...
        if jogador == 1:
            if not self._dialog_j1:
                ...
                self._dialog_j1   = MDDialog(
                    ...
                    buttons     = [MDFlatButton(
                        text=Textos.BTN_OK, on_release=lambda x: self._dialog_j1.dismiss()
                    )],
                )
            ...
            if not self._dialog_j2:
                ...
                self._dialog_j2   = MDDialog(
                    ...
                    content_cls = conteudo,
                    buttons     = [MDFlatButton(
                        text=Textos.BTN_OK, on_release=lambda x: self._dialog_j2.dismiss()
                    )],
                )
    ...
    def confirmar(self):
        """
        Valida os dados inseridos, salva as configurações no estado global
        e inicia a partida (transitando para a tela do tabuleiro).
        """
        nome1 = self.campo_j1.text.strip()
        nome2 = self.campo_j2.text.strip()
        if not nome1 or not nome2:
            Snackbar(text=Textos.ERRO_NOMES_VAZIOS).open()
            return
        if nome1.lower() == nome2.lower():
            Snackbar(text=Textos.ERRO_NOMES_IGUAIS).open()
            return
