...
class TabuleiroApp(MDApp):
    """
    Classe principal da aplicação, responsável por inicializar o KivyMD,
    configurar o gerenciador de telas (ScreenManager) e armazenar o estado global
    como o placar, as cores selecionadas e o controlador do jogo.
    """

    def __init__(self):
        """Inicializa as variáveis globais do aplicativo."""
        super().__init__()
        self.placar = None
        self.cor_j2 = None
        self.tipo_jogo = None
    ...
    def build(self):
        """
        Constrói a interface base da aplicação, define o tema inicial,
        instancia o controlador do jogo e adiciona as telas ao ScreenManager.
        
        Returns:
            ScreenManager: O gerenciador de telas principal do app.
        """
    ...
    def alternar_tema(self):
        """Alterna o tema do aplicativo entre Claro (Light) e Escuro (Dark)."""
    ...
    def get_tema_icon(self):
        """Retorna o ícone apropriado (sol ou lua) com base no tema atual."""
        return "weather-night" if self.theme_cls.theme_style == "Light" else "weather-sunny"

    def tema_action_item(self):
        """Retorna o item de ação para a AppBar com o ícone atual e o callback."""
        return [self.get_tema_icon(), lambda x: self._tema_e_atualiza()]

    def _tema_e_atualiza(self):
        """Função auxiliar para alternar o tema e notificar as telas e widgets para atualizar suas cores."""
        self.alternar_tema()
    ...
    def _forcar_atualizacao_textos(self, widget):
        """
        Percorre a árvore de widgets recursivamente forçando a atualização 
        das cores de textos e botões para que respeitem o novo tema (Claro/Escuro).
        
        Args:
            widget: O widget raiz a partir do qual a busca será iniciada.
        """
    ...
    def _carregar_placar(self):
        """
        Carrega o placar global a partir de um arquivo JSON.
        
        Returns:
            dict: Dicionário contendo vitórias do 'j1', 'j2' e 'empates'.
        """
        import json, os
    ...
    def salvar_placar(self):
        """Salva o placar global atual em um arquivo JSON local."""
        import json
        try:
